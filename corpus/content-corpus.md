...
**Consistency rules for any generated artifact:** cost figures are *planning estimates* (state it once); Azure is a *staging tier in front of* the Oracle EDW (never a replacement); the four options are *peers* on capability and differ on cost/effort/fit; the DIY verdict is *cheapest to run, most expensive to own*.

---

## 1. One-paragraph project context

The ERC Enterprise Data Warehouse is an Oracle Autonomous Data Warehouse on OCI, built as a four-workstream medallion (Bronze → Silver → Gold). Billow owns the in-EDW transformation (aiWorks + PL/SQL) that promotes staged data to Silver/Gold. This work designs the **upstream half** of the Whimsical "API & SSOR Data Ingress" board: an **Azure platform that ingests ~30 heterogeneous sources across three lanes, cleans them, ETLs them into the structures the logical model expects, and lands the finished table set into the OCI EDW Bronze/landing zone** where aiWorks takes over. The proposal evaluates four ways to build that Azure tier and prices them bottoms-up on live Azure retail rates.

---

## 2. The core framing (positioning statements — reuse verbatim)

- **Azure is an ingestion, quality, and staging tier that sits *in front of* the Oracle EDW — not a competing warehouse.** Its job is to hand clean, conformed, EDW-ready tables to Billow/aiWorks.
- **One contract line divides the world:** everything from source to the OCI Bronze landing is Azure's responsibility (this proposal); everything after — Silver, Gold, Power BI — is Billow/aiWorks. Because the contract is identical across every option, the platform choice is **reversible**.
...
**Next step.** A 2–3 week PoC landing two contrasting feeds end-to-end into OCI Bronze — GeoTab telematics (Lane 1: custom auth + streaming) and FastTrak or CGI Advantage (Lane 3: SFTP + XREF reconciliation) — with the `QUAR_`/manifest handoff to aiWorks. This converts every estimate to a committed number and de-risks the platform choice with data.
...
Evidence: the medallion target is Oracle; aiWorks + PL/SQL own Silver→Gold; the landing contract (`*_RAW`/`{TYPE}_STG`/`QUAR_`, ERC v3.0 naming) is identical across all options.
...
2. **The picture** — *Azure sits in front of the Oracle EDW.* Body: the source→Azure→OCI Bronze→aiWorks flow diagram. Talk: one contract line; everything left is this proposal.
