...
| **Assumptions & basis** | `model/ASSUMPTIONS.md` | Every volume/compute/storage/bandwidth/labor assumption behind the numbers. **Source for the basis slide (14) and the appendix (A5 ops decomposition / A7 risks & assumptions).** |
...
| A7 | **Risks & assumptions** | CARDS | Azure↔OCI connectivity · landing ownership boundary · VHSP PII · CB-03 · AssetWorks REST limits · Fabric sizing swings ~$60k · telematics volume drives the model · reservations not applied. Source: corpus §11 + `model/ASSUMPTIONS.md` §8 (load-bearing vs rounding error). |
