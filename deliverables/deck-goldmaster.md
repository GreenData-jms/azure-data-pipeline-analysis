...
**13 · Next step (light).** A 2–3 week two-feed PoC: GeoTab telematics (Lane 1) + FastTrak/CGI Advantage (Lane 3) → OCI staging with QUAR_/manifest hand-off to Agiline's aiWorks load (which runs Billow's PL/SQL). What it buys: committed numbers, validated staging→EDW hand-off, de-risked choice, parallel Fabric F-SKU trial.
